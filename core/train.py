...
def _log(step_count, loss_data, model, replay_buffer, test_score, best_test_score, lr, summary_writer):
    weighted_loss, loss, policy_loss, reward_loss, value_loss, reward_distribution, value_distribution = loss_data
    ...
    _msg = '#{:<10} Loss: {:<8.3f} [weighted Loss:{:<8.3f} Policy Loss: {:<8.3f} Value Loss: {:<8.3f} ' \
           'Reward Loss: {:<8.3f} ] Test Score: {:<10} Best Test Score: {:<10} ' \
           'Replay Episodes Collected: {:<10d} Buffer Size: {:<10d} Lr: {:<8.3f}'
    _msg = _msg.format(step_count, loss, weighted_loss, policy_loss, value_loss, reward_loss, str(test_score),
                       str(best_test_score), replay_episodes_collected, replay_buffer_size, lr)
    logger.info(_msg)
    ...
    if summary_writer is not None:
        ...
        summary_writer.add_histogram('training_data_distribution/reward', reward_distribution.flatten(), step_count)
        summary_writer.add_histogram('training_data_distribution/value', value_distribution.flatten(), step_count)
    ...
    def run(self):
        ...
        with torch.no_grad():
            while ray.get(self.shared_storage.get_counter.remote()) < self.config.training_steps:
                ...
                env = self.config.new_game(self.rank)
...
def update_weights(model, optimizer, replay_buffer, config):
    ...
    return weighted_loss.item(), loss.item(), policy_loss.mean().item(), reward_loss.mean().item(), \
           value_loss.mean().item(), target_reward, target_value
...
def _train(config, shared_storage, replay_buffer, summary_writer):
    ...
    for step_count in range(config.training_steps):
        ...
        loss_data = update_weights(model, optimizer, replay_buffer, config)

        test_score = None
        if step_count % config.test_interval == 0:
            test_model.set_weights(model.get_weights())
            test_score = test(config, test_model, config.test_episodes, config.device, False)
            if best_test_score is None or test_score >= best_test_score:
                best_test_score = test_score
                torch.save(model.state_dict(), config.model_path)

        _log(step_count, loss_data, model, replay_buffer, test_score, best_test_score, lr, summary_writer)
